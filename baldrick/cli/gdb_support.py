...
def installed_plugin_path() -> Path:
    """Return the entry-point script from the installed baldrick-gdb package."""
    spec = importlib.util.find_spec("baldrick_gdb")
    locations = spec.submodule_search_locations if spec is not None else None
    if not locations:
        raise FileNotFoundError(
            "Baldrick GDB plugin is not installed; run 'pip install baldrick-gdb'."
        )
    plugin = Path(next(iter(locations))) / "baldrick_gdb.py"
    if not plugin.is_file():
        raise FileNotFoundError("Installed baldrick-gdb package does not contain baldrick_gdb.py.")
    return plugin.resolve()


def register_gdb_commands(app: typer.Typer, console: Console) -> None:
    """Register commands integrating Baldrick with its separately shipped plugin."""
