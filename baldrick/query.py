"""
BaldrickQuery — synchronous Python API over the baldrick SQLite database.

Uses Polars + sqlite3 (Polars does not support aiosqlite).

Usage:
    from baldrick import BaldrickQuery
    import polars as pl

    q = BaldrickQuery("session.db")
...
from baldrick.queries import QueryDef, load_query_registry
...
class BaldrickQuery:
    """
    Convenience wrapper for ad-hoc Polars queries over a baldrick database.
