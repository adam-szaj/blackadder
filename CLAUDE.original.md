...
**DWARF deduplication**: `canonical_dwarf_type` stores each unique type (tag, name, byte_size, encoding)
exactly once globally. `binary_dwarf_ref` maps per-binary `die_offset` → `canonical_id`, preserving
`type_ref_die` for typedef/pointer chain resolution. Reduces ~12.8M rows → ~41k canonical + 12.8M lightweight refs.

## Built-in Queries (`baldrick query <name>`)

Full documentation with example output: `docs/queries.md`
