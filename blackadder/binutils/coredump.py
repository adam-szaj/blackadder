...
class CoreDumpParser:
    ...
    async def parse_core_dump(self, core_path: str) -> dict:
        ...
        headers_output = await self._get_readelf_output(
            core_path, ["-h"]
        )

        if not headers_output:
            raise ValueError(f"Failed to parse core dump headers: {core_path}")

        elf_headers = self.parse_elf_headers(headers_output)

        if elf_headers.get("type") != "ET_CORE":
            raise ValueError(f"File is not a core dump: {core_path}")

        # Step 2: Parse program headers
        prog_output = await self._get_readelf_output(
            core_path, ["-l"]
        )

        if not prog_output:
            raise ValueError(f"Failed to parse program headers: {core_path}")
    ...
    def extract_memory_segments(program_headers: list[dict]) -> list[dict]:
        ...
        for header in program_headers:
            ...
            # Create mapping
            mapping = {
                "start_addr": header["vaddr"],
                "end_addr": header["vaddr"] + header["memsz"],
                "perms": perms,
